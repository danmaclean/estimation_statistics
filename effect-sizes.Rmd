...
$$\bar{x_1} - \bar{x_2}$$
And $\bar{x}$ is a pretty standard name for a sample mean in the statistical literature.
...
So if we get a $d$ of 0.7, we have a medium to large effect size.

```{block2, type="callout"}
Computing $s$ is done according to a formula called the pooled standard deviation for two independent samples. If you don't look at lot's of formula it looks a little scary at first glance so I haven't included it here, I mention it just to point out that it _isn't_ the same as adding up the two individual sample standard deviations. 
```
